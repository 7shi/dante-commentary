...
**[公開ページ](https://7shi.github.io/dante-commentary/)** — 各カントを、原文と日本語訳を1行ずつ並べた対訳ページとして、解説記事の各節を該当する行範囲に埋め込んで掲載。三篇（地獄篇・煉獄篇・天国篇）ごとに、全カントへのリンクを示す索引ページも用意。

> [!NOTE]
> このリポジトリの解説記事・翻訳はすべて機械生成（LLM）であり、専門の翻訳者・研究者による校閲を経ていません。誤りや誤訳が随所に含まれている可能性があります。これらのファイルは、参考目的でそのまま提供されています。
