...
- 場面・主題ごとに見出しを付け、各見出しの下でその箇所の要点となる範囲を引用してください。見出しは10個前後を目安とします。
- 引用は行ごとに `> 行番号 原文` の次の行に `> （日本語訳）` を続ける形式にしてください。
- 連続する詩行はまとめて1つの引用ブロックにし、行と行の間は `>` のみの行で区切ってください。1詩行ごとにブロックを分けないでください。
- 引用に続けて、その行が示す寓意・背景の解説を地の文で書いてください。
- 引用は実際に論じる行だけに絞ってください。全行を引用する必要はありません。
- 1つの見出しの下でも、論じる箇所ごとに引用と地の文を交互に置いてください。見出しの担当範囲をまとめて引用してから解説する形にはしないでください。
- 最後に「## 結び――〈内容を要約した見出し〉」を置き、この歌全体が示す意味を短くまとめてください。
- 引用の日本語訳を除き、地の文は「です・ます調」で統一してください。
""".strip()
