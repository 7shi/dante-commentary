...
各スクリプトの詳しい使い方は本ファイルにまとめています。全体像は[プロジェクトの README](../README.md)を参照してください。

## generate.py

解説記事および翻訳テキストの生成スクリプトです（カント単位で処理し、セグメント分割は不使用）。
...
解説記事の引用ブロックを、原文コーパスと訳文テキストという正典から再構成し、正規の形式に統一するスクリプトです（後処理）。

解説記事の引用ブロック（`>` で始まる連続行のまとまり）には、原文行末の強制改行用スペース2つの欠落、訳文行末への不要な空白、原文と訳文の対応ズレ（生成時の付け間違い）などが混入することがあります。`fix_quote_blocks.py` は、ブロック内の `>` で始まる引用行をパターン判定し（数字始まりなら原文、`（` または `(` 始まりなら訳文）、以下の手順で処理します。

1. ブロックの先頭行が `> 行番号 ...` の形式でなければ、そのブロックはそのまま残します。
2. ブロック内の行番号の最小値・最大値を求め、その範囲を `dante_corpus`（原文）と対応する `<canticle>/<NN>.txt`（訳文）から取得します。
3. ブロック内の「原文行＋直後の訳文行」の各ペアを、行番号をキーに取得した原文・訳文で置き換えます。区切りの空行（`>` のみ）や行番号の飛び（部分引用）はそのまま保持します。
...
- 原文行は行末に半角スペース2つを付与します。
- 訳文行は `.txt` の該当行を全角括弧 `（）` で囲みます（`.txt` 側の鍵括弧などはそのまま保持されます）。
- 対応する `.txt` が存在しない場合、パスから篇・カント番号を特定できない場合は、ファイル単位で1回だけ警告を出力し、そのファイルは（ブロックの検査自体を行わずに）そのままスキップします。
- 訳文が見つからない（次行が訳文パターンに一致しない）場合、行番号が原文・訳文の範囲外の場合は、ブロックごとに警告を出力してそのペアをそのまま残します。

**重要**: この処理は単なる空白調整ではなく、行番号が一致する限り、記事内の引用テキスト自体を `dante_corpus`・`.txt` の内容で上書きします。原文と訳文の対応がズレていた箇所（例: 地獄篇第32歌6〜9行）も、この再構成によって正しい対応に修正されます。
...
| `-f`, `--format-only` | `dante_corpus`・`.txt` を参照せず、ファイル内に既にある引用ペアのレイアウト（強制改行用スペース、括弧の全角統一、区切り空行）のみを整形する（以前の挙動）。両モードの自動フォールバックはなく、明示的に指定した方だけが実行される | 再構成モード |

`--format-only` は `fable/` のように対応する `.txt` を持たないディレクトリでも動作します。

## check_style.py

解説記事が「です・ます調」か「だ・である調」かを判定するスクリプトです。
...
astra/inferno/03.md: です・ます調 (です・ます比率 125/125 = 100.0%)
```

実際の出力を確認したところ、比率は歌ごとに0%付近か90〜100%付近にはっきり分かれており、閾値付近の曖昧な判定は生じていません（現在は全カントが「です・ます調」に統一済みです）。

## check_quote_blocks.py

解説記事の各 `##` セクションに含まれる引用ブロック数を検査するスクリプトです。
